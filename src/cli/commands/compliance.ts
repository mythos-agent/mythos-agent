...
export async function complianceCommand(options: ComplianceOptions) {
  ...
  for (const report of reports) {
    const passColor = report.passRate >= 80 ? chalk.green : report.passRate >= 60 ? chalk.yellow : chalk.red;
    console.log(
    ...
    if (failed.length > 0) {
      console.log(chalk.red(`    ❌ ${failed.length} failed: ${failed.map((c) => c.controlId).join(", ")}`));
    }
    if (partial.length > 0) {
      console.log(chalk.yellow(`    ⚠️  ${partial.length} partial: ${partial.map((c) => c.controlId).join(", ")}`));
    }
    console.log();
  }
}
