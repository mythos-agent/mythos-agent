...
interface SummaryOptions { path?: string; }
...
export async function summaryCommand(options: SummaryOptions) {
  ...
  const bar = (count: number, max: number, color: (s: string) => string) => {
    ...
    return count > 0 ? color("█".repeat(width)) + chalk.dim("░".repeat(20 - width)) : chalk.dim("░".repeat(20));
  };
  const maxCount = Math.max(counts.critical, counts.high, counts.medium, counts.low, 1);

  console.log(`  Critical  ${String(counts.critical).padStart(3)}  ${bar(counts.critical, maxCount, chalk.red)}`);
  console.log(`  High      ${String(counts.high).padStart(3)}  ${bar(counts.high, maxCount, chalk.yellow)}`);
  console.log(`  Medium    ${String(counts.medium).padStart(3)}  ${bar(counts.medium, maxCount, chalk.blue)}`);
  console.log(`  Low       ${String(counts.low).padStart(3)}  ${bar(counts.low, maxCount, chalk.dim)}`);
  ...
  if (history.scans.length >= 2) {
    ...
    console.log(`    Findings: ${delta <= 0 ? chalk.green(`${delta}`) : chalk.red(`+${delta}`)}   Score: ${scoreDelta >= 0 ? chalk.green(`+${scoreDelta.toFixed(1)}`) : chalk.red(scoreDelta.toFixed(1))}`);
  }
  ...
  if (result) {
    ...
    if (diff) {
      console.log(chalk.bold("\n  Baseline Comparison"));
      console.log(`    New: ${diff.newFindings.length > 0 ? chalk.red(`+${diff.newFindings.length}`) : chalk.green("0")}   Fixed: ${diff.fixedFindings.length > 0 ? chalk.green(`-${diff.fixedFindings.length}`) : chalk.dim("0")}   Unchanged: ${diff.unchangedCount}`);
    }
  }
  ...
  if (policy && result) {
    ...
    console.log(`    Status: ${policyResult.passed ? chalk.green("PASSED") : chalk.red("FAILED")}   Violations: ${policyResult.violations.length}   Warnings: ${policyResult.warnings.length}`);
  }

  // Tools
  const installed = tools.filter((t) => t.installed);
  console.log(chalk.bold("\n  Tools") + chalk.dim(` (${installed.length}/${tools.length})`));
  console.log(`    ${installed.map((t) => chalk.green(t.name)).join("  ") || chalk.dim("none installed")}`);

  // Config
  console.log(chalk.bold("\n  Config"));
  console.log(chalk.dim(`    Provider: ${config.provider}   Model: ${config.model}`));
  console.log(chalk.dim(`    Config: ${configFile ? "yes" : "no"}   API key: ${config.apiKey ? "yes" : "no"}`));

  // Quick actions
  console.log(chalk.bold("\n  Quick Actions"));
  if (counts.critical > 0) console.log(chalk.cyan("    sphinx-agent fix --severity critical --apply"));
