...
const OAUTH_RULES: Array<{
...
}> = [
  {
    ...
    description:
      "OAuth authorization request without state parameter. This enables CSRF attacks on the login flow.",
    severity: "high",
    cwe: "CWE-352",
    patterns: [
  ...
  },
  {
    ...
    description:
      "OAuth implicit flow (response_type=token) returns tokens in URL fragment. Use authorization code flow with PKCE.",
    severity: "high",
    cwe: "CWE-319",
    patterns: [
  ...
  },
  {
    ...
    description:
      "OAuth authorization code flow without PKCE. Public clients (SPAs, mobile) must use PKCE to prevent code interception.",
    severity: "medium",
    cwe: "CWE-287",
    patterns: [
  ...
  },
  {
    ...
    description:
      "Access token passed as URL query parameter. Tokens in URLs leak via Referer headers, browser history, and logs.",
    severity: "high",
    cwe: "CWE-598",
    patterns: [
      /\?.*access_token=/gi,
  ...
  },
  {
    ...
    description:
      "OAuth redirect_uri not validated against whitelist. Attacker can redirect tokens to their own server.",
    severity: "critical",
    cwe: "CWE-601",
    patterns: [
  ...
  },
  {
    ...
    description:
      "OAuth client_secret in client-side code. Client secrets must only exist on the server. Use PKCE for public clients.",
    severity: "critical",
    cwe: "CWE-798",
    patterns: [
  ...
  },
  {
    ...
    description:
      "OAuth token used without checking expiration. Expired tokens should be refreshed, not silently accepted.",
    severity: "medium",
    cwe: "CWE-613",
    patterns: [
...
export class OauthScanner {
  async scan(projectPath: string): Promise<OauthScanResult> {
    ...
    for (const file of files) {
      ...
      if (
        !/oauth|oidc|authorize|token|client_id|client_secret|redirect_uri|passport|auth0|okta|cognito/i.test(
          content
        )
      )
        continue;
      ...
      for (const rule of OAUTH_RULES) {
        for (const p of rule.patterns) {
          ...
          if (match) {
            ...
            findings.push({
              id: `OAUTH-${String(id++).padStart(4, "0")}`,
              rule: `oauth:${rule.id}`,
              title: rule.title,
