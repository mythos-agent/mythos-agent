...
const BIZ_RULES: Array<{ id: string; title: string; description: string; severity: Severity; cwe: string; patterns: RegExp[] }> = [
  {
    id: "biz-negative-amount",
    title: "Business Logic: No Negative Amount Validation",
    description: "Payment/transfer amount used without checking for negative values. Negative amounts can reverse transactions.",
    severity: "high",
    cwe: "CWE-20",
    patterns: [
  ...
  },
  {
    ...
    description: "Discount or coupon code applied without checking usage count. Same code can be used unlimited times.",
    severity: "medium",
    cwe: "CWE-799",
    patterns: [
  ...
  },
  {
    ...
    description: "Item purchased without locking inventory. Concurrent purchases can oversell stock.",
    severity: "high",
    cwe: "CWE-362",
    patterns: [
  ...
  },
  {
    ...
    description: "API allows users to set their own role/permissions. Role changes must be admin-only operations.",
    severity: "critical",
    cwe: "CWE-269",
    patterns: [
  ...
  },
  {
    ...
    description: "Account actions available without checking email verification status. Users can act on unverified accounts.",
    severity: "medium",
    cwe: "CWE-863",
    patterns: [
  ...
  },
  {
    ...
    description: "Rate limiting only checks IP address. Attackers behind proxies or VPNs bypass this. Also rate-limit by user ID.",
    severity: "medium",
    cwe: "CWE-799",
    patterns: [
...
export interface BusinessLogicScanResult { findings: Vulnerability[]; filesScanned: number; }
      ...
      for (const rule of BIZ_RULES) {
        for (const p of rule.patterns) {
          ...
          if (match) {
            ...
            findings.push({ id: `BIZ-${String(id++).padStart(4, "0")}`, rule: `biz:${rule.id}`, title: rule.title, description: rule.description, severity: rule.severity, category: "business-logic", cwe: rule.cwe, confidence: "medium", location: { file: rel, line: ln, snippet: lines[ln - 1]?.trim() || "" } });
            break;
          }
        }
      }
