...
const SSTI_RULES: Array<{
...
}> = [
  {
    ...
    description:
      "User input passed to Jinja2 Template() or render_template_string(). Enables server-side code execution.",
    severity: "critical",
    cwe: "CWE-1336",
    patterns: [
  ...
  },
  {
    ...
    description:
      "User input in EJS render. Use <%- for escaped output, never pass user input as template string.",
    severity: "critical",
    cwe: "CWE-1336",
    patterns: [
  ...
  },
  {
    ...
    description:
      "User input compiled as Handlebars template. Use precompiled templates with data context instead.",
    severity: "critical",
    cwe: "CWE-1336",
    patterns: [
  ...
  },
  {
    ...
    description:
      "User input in Pug template compilation. Pug templates can execute arbitrary JavaScript.",
    severity: "critical",
    cwe: "CWE-1336",
    patterns: [
  ...
  },
  {
    ...
    severity: "critical",
    cwe: "CWE-1336",
    patterns: [/nunjucks\.renderString\s*\(\s*(?:req|input|data|body|user)/gi],
  },
  {
    id: "ssti-twig",
    title: "SSTI: Twig Template with User Input (PHP)",
    description:
      "User input in Twig template creation. Use Twig's auto-escaping and never create templates from user input.",
    severity: "critical",
    cwe: "CWE-1336",
    patterns: [
  ...
  },
  {
    ...
    description:
      "User input parsed as Go template. template.New().Parse() with user input enables code execution.",
    severity: "critical",
    cwe: "CWE-1336",
    patterns: [
...
export class SstiScanner {
  async scan(projectPath: string): Promise<SstiScanResult> {
    ...
    for (const file of files) {
      ...
      if (!/template|render|compile|jinja|ejs|handlebars|pug|jade|nunjucks|twig/i.test(content))
        continue;
      ...
      for (const rule of SSTI_RULES) {
        for (const p of rule.patterns) {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (p.test(lines[i])) {
              findings.push({
                id: `SSTI-${String(id++).padStart(4, "0")}`,
                rule: `ssti:${rule.id}`,
                title: rule.title,
