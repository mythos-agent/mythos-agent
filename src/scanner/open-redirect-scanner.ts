...
const REDIRECT_RULES: Array<{
...
}> = [
  {
    ...
    description:
      "Redirect destination from user input without validation. Attacker redirects users to phishing sites after login.",
    severity: "medium",
    cwe: "CWE-601",
    patterns: [
  ...
  },
  {
    ...
    description:
      "Redirect allows protocol-relative URLs (//evil.com). These redirect to external sites while bypassing domain checks.",
    severity: "medium",
    cwe: "CWE-601",
    patterns: [
  ...
  },
  {
    ...
    description:
      "Setting Location header with user input. Can enable header injection if newlines aren't stripped.",
    severity: "high",
    cwe: "CWE-113",
    patterns: [
  ...
  },
  {
    ...
    description:
      "HTML meta refresh tag with user-controlled URL. Same impact as server-side open redirect.",
    severity: "medium",
    cwe: "CWE-601",
    patterns: [
...
export interface OpenRedirectScanResult {
      ...
      for (const rule of REDIRECT_RULES) {
        for (const p of rule.patterns) {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (p.test(lines[i])) {
              findings.push({
                id: `REDIR-${String(id++).padStart(4, "0")}`,
                rule: `redirect:${rule.id}`,
                title: rule.title,
