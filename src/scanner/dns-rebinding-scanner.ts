...
const DNS_RULES: Array<{ id: string; title: string; description: string; severity: Severity; cwe: string; patterns: RegExp[] }> = [
  {
    id: "dns-localhost-only-no-host-check",
    title: "DNS Rebinding: Localhost Service Without Host Validation",
    description: "Service binds to localhost/127.0.0.1 but does not validate the Host header. DNS rebinding can bypass same-origin policy to access it.",
    severity: "high",
    cwe: "CWE-350",
    patterns: [
  ...
  },
  {
    ...
    description: "CORS configured to allow localhost origins. Combined with DNS rebinding, any website can access this API.",
    severity: "medium",
    cwe: "CWE-942",
    patterns: [
  ...
  },
  {
    ...
    description: "API bound to localhost without authentication. If DNS rebinding is possible, external sites can access it.",
    severity: "high",
    cwe: "CWE-306",
    patterns: [
  ...
  },
  {
    ...
    description: "Development server bound to all interfaces (0.0.0.0). This exposes it to the network, not just localhost.",
    severity: "medium",
    cwe: "CWE-668",
    patterns: [
...
export interface DnsRebindingScanResult { findings: Vulnerability[]; filesScanned: number; }
      ...
      for (const rule of DNS_RULES) {
        for (const p of rule.patterns) {
          ...
          if (match) {
            ...
            findings.push({ id: `DNS-${String(id++).padStart(4, "0")}`, rule: `dns:${rule.id}`, title: rule.title, description: rule.description, severity: rule.severity, category: "dns-rebinding", cwe: rule.cwe, confidence: "medium", location: { file: rel, line: ln, snippet: lines[ln - 1]?.trim() || "" } });
            break;
          }
        }
      }
