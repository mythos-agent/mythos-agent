...
const PATH_RULES: Array<{
...
}> = [
  {
    ...
    description:
      "path.join does NOT prevent traversal. '../../../etc/passwd' passes through. Use path.resolve + startsWith check.",
    severity: "high",
    cwe: "CWE-22",
    patterns: [/path\.join\s*\(.*(?:req\.|params\.|query\.|body\.|input|user)/gi],
  },
  {
