...
|---|---|---|---|---|
| **mythos-agent v4.0.1** (deterministic scanners only) | _pending_ | _pending_ | _pending_ | **1 / 5** |
...
The `1 / 5` CVE Replay number for v4.0.1 — after RedosScanner's
template-literal extraction improvement:

| GHSA | CVE | Package | Bug class | Status | Notes |
|---|---|---|---|---|---|
| GHSA-c2qf-rxjj-qqgw | CVE-2022-25883 | semver | ReDoS | **caught** | Template-literal extractor + new "unbounded whitespace adjacent to template interpolation" indicator — the scanner change that moved this from miss to catch |
...
| GHSA-cxjh-pqwp-8mfp | CVE-2024-28849 | follow-redirects | Cross-host credential forwarding | missed | No deterministic scanner covers this class; needs a new rule |
...
`fix-clean: 4 / 5`. Four cases have no finding at the fix commit. The
fifth (semver) fix-clean is intentionally **NO**: upstream fixed
...
and the fix-clean NO is itself useful signal that the upstream fix
was a partial mitigation, not a full remediation. Re-run with
