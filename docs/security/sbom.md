...
How sphinx-agent produces, distributes, and maintains its Software Bill of Materials.

## What we publish

For every release of sphinx-agent, we publish a CycloneDX SBOM in two formats:

| Artifact | Format | Spec version |
|---|---|---|
| `sphinx-agent-sbom.cdx.json` | CycloneDX JSON | 1.6 |
| `sphinx-agent-sbom.cdx.xml` | CycloneDX XML | 1.6 |
...
- The sphinx-agent npm package itself
...
- External tools that sphinx-agent invokes via subprocess (Semgrep, Trivy, Gitleaks, Checkov, Nuclei) — these are user-installed and tracked in their own SBOMs
...
cosign verify-blob \
  --certificate sphinx-agent-sbom.cdx.json.crt \
  --signature   sphinx-agent-sbom.cdx.json.sig \
  --certificate-identity-regexp 'https://github.com/zhijiewong/sphinx-agent/.github/workflows/sbom\.yml@.*' \
  --certificate-oidc-issuer 'https://token.actions.githubusercontent.com' \
  sphinx-agent-sbom.cdx.json
```
...
Under the EU Cyber Resilience Act, a Manufacturer placing a product on the EU market must include an SBOM covering the product's components. When you integrate sphinx-agent into such a product, our SBOM gives you:
