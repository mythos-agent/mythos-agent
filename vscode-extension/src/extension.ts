...
export function activate(context: vscode.ExtensionContext) {
  diagnosticCollection =
    vscode.languages.createDiagnosticCollection("mythos-agent");
  ...
  statusBarItem.command = "mythos-agent.scanWorkspace";
  statusBarItem.text = "$(shield) mythos-agent";
  statusBarItem.tooltip = "Click to scan workspace";
  statusBarItem.show();

  // Commands
  context.subscriptions.push(
    vscode.commands.registerCommand("mythos-agent.scanFile", async () => {
    ...
    vscode.commands.registerCommand("mythos-agent.scanWorkspace", async () => {
      await vscode.window.withProgress(
        {
          location: vscode.ProgressLocation.Notification,
          title: "mythos-agent: Scanning workspace...",
          cancellable: false,
        },
        async () => {
...
function updateStatusBar() {
  ...
  if (total === 0) {
    statusBarItem.text = "$(shield) mythos-agent: Clean";
    statusBarItem.backgroundColor = undefined;
  } else if (critical > 0) {
    statusBarItem.text = `$(shield) mythos-agent: ${critical} critical, ${total} total`;
    statusBarItem.backgroundColor = new vscode.ThemeColor(
      "statusBarItem.errorBackground"
    );
  } else {
    statusBarItem.text = `$(shield) mythos-agent: ${total} issues`;
    statusBarItem.backgroundColor = new vscode.ThemeColor(
...
class SphinxCodeActionProvider implements vscode.CodeActionProvider {
  provideCodeActions(
    ...
    for (const diagnostic of context.diagnostics) {
      if (diagnostic.source !== "mythos-agent") continue;

      // AI Fix action
      const fixAction = new vscode.CodeAction(
        `mythos-agent: Fix with AI`,
        vscode.CodeActionKind.QuickFix
      );
      fixAction.command = {
        command: "mythos-agent.fixVulnerability",
        title: "Fix with AI",
      ...
      const suppressAction = new vscode.CodeAction(
        `mythos-agent: Suppress this warning`,
        vscode.CodeActionKind.QuickFix
      );
      ...
      suppressAction.edit.insert(
        document.uri,
        line.range.start,
        `// mythos-agent-ignore: ${diagnostic.code}\n`
      );
