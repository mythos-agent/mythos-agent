...
Thanks for your interest. sphinx-agent — the **Sphinx Mythos** — is an open-source AI security agent. We welcome code, rules, tests, docs, integrations, and ideas.
...
>
> **Looking for the strategic context?** [VISION.md](VISION.md) is the north star; [ROADMAP.md](ROADMAP.md) is the multi-year frame; the pinned issue **`[Roadmap] sphinx-agent H1 2026 Goals`** is the active 6-month plan.

## Quick Start

```bash
git clone https://github.com/zhijiewong/sphinx-agent.git
cd sphinx-agent
npm install
npm run build
npm test
...
The `sphinx-rules-*` npm prefix is reserved for community rule packs. The forthcoming **scanner plugin SDK** (Q3 2026 milestone in ROADMAP) will document a programmatic alternative to YAML rules for cases that need real logic.
...
- The auto-updated [Sphinx Mythos Pioneers leaderboard](docs/pioneers.md) (lands H1 2026)
...
sphinx-agent is a defensive tool. By contributing, you agree to use it — and to design contributions that — primarily serve defenders.

- **Do not** use sphinx-agent to scan systems you do not own or have explicit written permission to test.
...
- **Responsibly disclose** any zero-day vulnerability that sphinx-agent helps you find — do not weaponize it.
- The sphinx-agent maintainers reserve the right to decline contributions that materially shift the project's defensive posture.
